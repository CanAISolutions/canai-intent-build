...
export const generateSparkSplit = async (request: SparkSplitRequest): Promise<SparkSplitResponse> => {
  try {
...
export const submitFeedback = async (request: FeedbackRequest): Promise<FeedbackResponse> => {
  try {
    ...
    localStorage.setItem('canai_sparksplit_feedback', JSON.stringify({
      ...request,

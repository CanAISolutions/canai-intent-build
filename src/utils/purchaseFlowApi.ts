...
export const createStripeSession = async (data: StripeSessionRequest): Promise<StripeSessionResponse> => {
  ...
  try {
    ...
    console.warn('[Purchase API] createStripeSession failed, using fallback:', error);
...
// Process refund
export const processRefund = async (data: RefundRequest): Promise<RefundResponse> => {
...
export const switchProduct = async (data: SwitchProductRequest): Promise<SwitchProductResponse> => {
...
export const logPaymentInitiation = async (paymentData: {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[F4-E1] Payment logging attempt ${attempt + 1} failed:`, error);
      ...
      if (attempt === maxRetries) {
        ...
        try {
          ...
          const existingData = JSON.parse(localStorage.getItem('canai_payment_fallback') || '[]');
          existingData.push(fallbackData);
          localStorage.setItem('canai_payment_fallback', JSON.stringify(existingData));

...
export const getGenerationStatus = async (promptId: string): Promise<GenerationStatusResponse> => {
  console.log('[DeliverableAPI] GET /v1/generation-status called with promptId:', promptId);
  
  try {
    const startTime = Date.now();
    
    // TODO: Replace with actual backend endpoint
    const response = await deliverableApiCall<GenerationStatusResponse>(`/generation-status?promptId=${promptId}`);
    
    const duration = Date.now() - startTime;
    console.log(`[DeliverableAPI] Generation status retrieved in ${duration}ms:`, response);
    
    return response;
    
  } catch (error) {
    console.warn('[DeliverableAPI] getGenerationStatus failed, using fallback:', error);
...
export const requestRevision = async (data: RevisionRequest): Promise<RevisionResponse> => {
  ...
  try {
    ...
    const response = await deliverableApiCall<RevisionResponse>('/request-revision', {
      method: 'POST',
    ...
    if (duration > 2000) {
      console.warn(`[DeliverableAPI] request-revision exceeded 2s target: ${duration}ms`);
    }
    
    console.log(`[DeliverableAPI] Revision completed in ${duration}ms:`, response);
    return response;
...
export const regenerateDeliverable = async (data: RegenerateRequest): Promise<RegenerateResponse> => {
  console.log('[DeliverableAPI] POST /v1/regenerate-deliverable called with:', data);
  
  try {
    const startTime = Date.now();
    
    // TODO: Replace with actual GPT-4o integration
    const response = await deliverableApiCall<RegenerateResponse>('/regenerate-deliverable', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    ...
    // Ensure response time < 2s target
    if (duration > 2000) {
      console.warn(`[DeliverableAPI] regenerate-deliverable exceeded 2s target: ${duration}ms`);
    }
    
    console.log(`[DeliverableAPI] Regeneration completed in ${duration}ms:`, response);
    return response;
...
export const validateEmotionalResonance = async (content: string): Promise<HumeResonanceResponse> => {
  ...
  try {
    ...
    console.log(`[DeliverableAPI] Hume AI validation completed in ${duration}ms`);
...
export const generateDeliverableContent = async (
...
): Promise<{ canaiOutput: string; genericOutput: string; emotionalResonance: HumeResonanceResponse }> => {
  ...
  try {
    ...
    if (openaiApiKey === 'demo-key') {
      ...
      const genericOutput = generateMockContent(productType, businessInputs, false);
      ...
      fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: 'user',
              content: `Business type: ${businessInputs.businessType || 'general business'}`
            }
          ],
          max_tokens: 1000,
          temperature: 0.3
    ...
    const duration = Date.now() - startTime;
    console.log(`[DeliverableAPI] Content generation completed in ${duration}ms`);
...
const generateMockContent = (productType: string, inputs: Record<string, any>, personalized: boolean): string => {
  const businessName = inputs.businessName || 'Your Business';
  const targetAudience = inputs.targetAudience || 'target customers';
  
  if (!personalized) {
    return `Generic ${productType.replace('_', ' ').toLowerCase()} template with standard recommendations and placeholder content.`;
  }

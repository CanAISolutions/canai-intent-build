...
const MEMBERSTACK_PUBLIC_KEY = import.meta.env.VITE_MEMBERSTACK_PUBLIC_KEY || 'pk_demo_key';
...
class MemberstackAuthClient {
  ...
  private async initializeMemberstack() {
    try {
      ...
      if (typeof window !== 'undefined' && MEMBERSTACK_PUBLIC_KEY !== 'pk_demo_key') {
        ...
        console.log('[Memberstack] Initialization skipped - using fallback mode');
      }
    } catch (error) {
      console.warn('[Memberstack] Initialization failed, using fallback mode:', error);
  ...
  async checkDeliverableAccess(productType: string): Promise<boolean> {
    ...
    try {
      ...
      const hasActivePlan = user.planConnections?.some(plan => 
        plan.status === 'ACTIVE' && 
        ['business_builder', 'social_email', 'site_audit'].includes(productType.toLowerCase())
      );

      console.log(`[Memberstack] Deliverable access check for ${productType}:`, hasActivePlan);
  ...
  async getAuthStatus(): Promise<MemberstackAuthStatus> {
    try {
      ...
      const hasDeliverableAccess = authenticated ? await this.checkDeliverableAccess('business_builder') : false;
...
// Helper functions
export const requireDeliverableAccess = async (productType: string): Promise<boolean> => {
...
export const trackUserAccess = async (action: string, details?: Record<string, any>) => {

...
import { requireDeliverableAccess } from "@/utils/memberstackAuth";
...
const SparkSplit: React.FC = () => {
  ...
  const fetchSparkSplit = useCallback(
    async (retry = 0) => {
      ...
      // Check Memberstack access
      const hasAccess = await requireDeliverableAccess('business_builder');
      if (!hasAccess) {
      ...
      try {
        // Call the real SparkSplit API
        const response = await generateSparkSplit({
          canaiOutput: demoCanaiOutput,
          genericOutput: '', // Will be generated by GPT-4o
          prompt_id: promptId,
        });
        ...
        setEmotionalResonance({
          canaiScore: response.emotionalResonance.canaiScore,
          genericScore: response.emotionalResonance.genericScore,
          delta: response.emotionalResonance.delta,
          arousal: response.emotionalResonance.arousal,
          valence: response.emotionalResonance.valence,
          compassScores: {
            awe: 0.85,
            ownership: 0.92,
            wonder: 0.78,
        ...
        trackEvent("plan_compared", {
          ...
          selected: null,
          emotionalResonance: { delta: response.emotionalResonance.delta },
          completion_time_ms: loadTime,
          prompt_id: promptId,
        });

        if (loadTime > 500) {
          console.warn(`SparkSplit load time exceeded 500ms: ${loadTime}ms`);
        }
  ...
  const handleTrustDeltaView = () => {
    ...
    if (tooltipTime > 100) {
      console.warn(`Trust Delta tooltip exceeded 100ms: ${tooltipTime}ms`);
    }
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Log feedback to SparkSplit API and Supabase
      if (trustDelta && emotionalResonance) {

...
const EmotionalCompass: React.FC<EmotionalCompassProps> = ({ scores, title = "Emotional Resonance" }) => {
  ...
  useEffect(() => {
    ...
    const values = [scores.awe, scores.ownership, scores.wonder, scores.calm, scores.power];
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <svg
        ...
        role="img"
        aria-label={`Emotional compass showing scores for Awe: ${(scores.awe * 100).toFixed(0)}%, Ownership: ${(scores.ownership * 100).toFixed(0)}%, Wonder: ${(scores.wonder * 100).toFixed(0)}%, Calm: ${(scores.calm * 100).toFixed(0)}%, Power: ${(scores.power * 100).toFixed(0)}%`}
      />
    </div>
  );
};

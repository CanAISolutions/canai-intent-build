...
const StandardFormGroup = React.forwardRef<HTMLDivElement, StandardFormGroupProps>(
  ({ className, children, required, error, success, validationState = "idle", ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          ...
          hasBeenTouched && validationState === "error" && "animate-[shake_0.2s_ease-in-out]",
          className
        )}
        {...props}
      >
...
>(({ className, required, children, ...props }, ref) => (
  ...
  >
    {children}
    {required && <span className="text-[#36d1fe] ml-1" aria-label="required">*</span>}
  </Label>
));
...
>(({ className, error, success, validationState = "idle", onValidationChange, ...props }, ref) => {

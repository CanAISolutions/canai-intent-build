...
export const ValidatedForm = React.forwardRef<HTMLFormElement, ValidatedFormProps>(
  ({ className, fields, onValidSubmit, variant = "default", children, onSubmit, ...props }, ref) => {
    const { validations, getFieldProps, isFormValid } = useFormValidation(fields);

    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      
      if (isFormValid() && onValidSubmit) {
        const formData = Object.keys(validations).reduce((acc, key) => ({
          ...acc,
...
export const ValidatedField = React.forwardRef<HTMLDivElement, ValidatedFieldProps>(
  ({ name, label, type = "text", placeholder, helperText, required, validation, className }, ref) => {
      ...
      >
        ...
        {type === "textarea" ? (
          <StandardFormTextarea
            ...
            aria-describedby={helperText ? `${name}-helper` : fieldProps.error ? `${name}-error` : undefined}

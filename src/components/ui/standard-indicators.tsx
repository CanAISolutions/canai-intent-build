  ...
  ({ className, size = "md", text, ...props }, ref) => {
    ...
    return (
      <div
        ...
        {...props}
      >
        <Loader2 className={cn("animate-spin text-[#36d1fe]", sizeClasses[size])} />
        {text && (
...
// Status Badge
interface StandardStatusBadgeProps extends React.HTMLAttributes<HTMLDivElement> {
  variant?: "success" | "error" | "warning" | "info" | "pending";
  size?: "sm" | "md" | "lg";
  showIcon?: boolean;
}

const StandardStatusBadge = React.forwardRef<HTMLDivElement, StandardStatusBadgeProps>(
...
const StandardProgress = React.forwardRef<HTMLDivElement, StandardProgressProps>(
  ({ className, value, max = 100, showPercentage = false, variant = "default", size = "md", ...props }, ref) => {
...
const StandardTrustScore = React.forwardRef<HTMLDivElement, StandardTrustScoreProps>(
  ({ className, score, maxScore = 5, showLabel = true, variant = "detailed", ...props }, ref) => {
            ...
            {stars.map((star) => (
              <div
                ...
                className={cn(
                  ...
                  star <= score ? "bg-[#36d1fe] shadow-[0_0_4px_#36d1fe]" : "bg-[rgba(255,255,255,0.2)]"
                )}
              />
            ))}
          </div>
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-white font-medium font-manrope">Trust Score</span>

...
export const usePerformanceMonitor = (
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (loadTime > targetLoadTime) {
        console.warn(`[Performance] ${pageName} exceeded ${targetLoadTime}ms target`);
      }
    }

    return () => {
      // Track time spent on page
      const timeOnPage = performance.now() - (renderTimeRef.current || startTimeRef.current);
  ...
  const trackInteraction = (interactionName: string) => {
    const interactionTime = performance.now() - (renderTimeRef.current || startTimeRef.current);

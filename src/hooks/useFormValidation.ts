...
export const useFormValidation = (fields: Record<string, ValidationRule>) => {
  ...
  const updateField = useCallback((fieldName: string, value: string) => {
    setValidations(prev => {
      ...
      const state: ValidationState = error ? "error" : value.trim() ? "success" : "idle";
  ...
  const isFormValid = useCallback(() => {
    return Object.values(validations).every(field => 
      field.state === "success" || (field.state === "idle" && !fields[Object.keys(validations).find(key => validations[key] === field)!]?.required)
    );
  }, [validations, fields]);
